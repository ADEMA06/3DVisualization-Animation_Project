...
VSShaderLib *shaderFlare = new VSShaderLib();
...
void drawObjects(bool repeated) {
	...
	if (road.carPassesFlag(car_pos)) {
		Texture2D_Loader(TextureArray, "sand.jpg", 0);
		Texture2D_Loader(TextureArray, "sand.jpg", 2);
...
void setLights() {
	...
	for (int i = 0; i < candles.size(); i++) {
		...
		if (i == 0) lightPos[0] = mult[0], lightPos[1] = mult[1], lightPos[2] = mult[2];
...
void render_flare(FLARE_DEF* flare, int lx, int ly, int* m_viewport, VSShaderLib* shader) {  //lx, ly represent the projected position of light on viewport
	...
	GLuint programIndex = shader->getProgramIndex();
	...
	loc = glGetUniformLocation(shader->getProgramIndex(), "texmap8");
	glUniform1i(loc, 0);  //use TU 0
	...
	for (i = 0; i < flare->nPieces; ++i)
	{
		...
		if (width > 1)
		{
			...
			loc = glGetUniformLocation(shader->getProgramIndex(), "m_viewModel");
			glUniformMatrix4fv(loc, 1, GL_FALSE, mCompMatrix[VIEW_MODEL]);
			loc = glGetUniformLocation(shader->getProgramIndex(), "m_pvm");
			glUniformMatrix4fv(loc, 1, GL_FALSE, mCompMatrix[PROJ_VIEW_MODEL]);
			computeNormalMatrix3x3();
			loc = glGetUniformLocation(shader->getProgramIndex(), "m_normal");
			glUniformMatrix3fv(loc, 1, GL_FALSE, mNormal3x3);
...
void init()
{
	...
	Texture2D_Loader(TextureArray, "vulcanic_ground.png", 0);
	Texture2D_Loader(TextureArray, "orange.jpg", 1);
	Texture2D_Loader(TextureArray, "vulcanic_ground.png", 2);
